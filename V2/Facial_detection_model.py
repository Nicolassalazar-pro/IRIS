...
DISPLAY_CONFIG = {
    'SHOW_CAMERA': False,      # Toggle camera window display
    'SHOW_BOUNDING_BOX': False,  # Toggle face bounding box display
    'SHOW_LABELS': False,      # Toggle name/ID labels display
}
...
def face_detect(camera, face_encodings, face_identifiers, awaiting_first_enrollment, collection):
    while True:
        ...
        if DISPLAY_CONFIG['SHOW_CAMERA'] and display_frame is not None:
...
def main():
    ...
    FaceID_Module = threading.Thread(
        target=face_detect, 
        args=(camera, face_encodings, face_identifiers, awaiting_first_enrollment, collection)
    )
