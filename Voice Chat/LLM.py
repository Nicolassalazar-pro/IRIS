...
def get_ollama_response(messages):
    """Get response from Ollama with personality context"""
    try:
        # Create a messages list that leads with personality as system message
        personality_messages = [
            {"role": "system", "content": FRIDAY_PERSONALITY},
            # Add a reinforcement message to maintain personality
            {"role": "system", "content": "Remember: Keep responses short and direct. No emojis or multiple questions."},
            # Include last 5 conversation messages
            *messages[-5:]
        ]
            ...
            with contextlib.redirect_stdout(devnull), contextlib.redirect_stderr(devnull):
                response = ollama.chat(
                    ...
                    options={
                        ...
                        "num_ctx": 2048,
                        "num_thread": 6,
                        "temperature": 0.4,  # Lowered for more consistent personality
                        "top_k": 40,
                        "top_p": 0.9
                    }
                )
